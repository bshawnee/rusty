...
use postgres_protocol::message::backend::{Message, self};
...
use crate::monitoring::{ActiveQueries, QueryExecution, QueryLogEntry};

static QUERY_COUNTER_ID: AtomicU64 = AtomicU64::new(0);

struct QueryCounterId;

impl QueryCounterId {
    fn get_next() -> u64 {
        QUERY_COUNTER_ID.fetch_add(1, Ordering::SeqCst)
    }
}
...
impl PostgresParser {
    ...
    fn handle_client_message(&mut self, msg_type: u8, body: &[u8]) {
        match msg_type {
            ...
            b'p' => debug!(client = %self.client_addr, "Password sent (hidden)"),
            _ => {
                debug!(
                    client = %self.client_addr,
    ...
    fn start_query(&mut self, query: &str, query_type: QueryType) {
        self.query_counter = QueryCounterId::get_next();
    ...
    fn handle_server_message_parsed(&mut self, message: Message) {
        match message {
            ...
            // Остальные сообщения аналогично...
            _ => {
                debug!(client = %self.client_addr, "Server message");
            }
        }
    }
